...
def play():
    ...
    est = setANewState(table, info)
    ret = estimate(info["player_turn"], est[0], est[1], 0)
    print("Heuristika za ovaj potez je: ")
    print(ret)
...
def getAllPossibleMoves(turn, table_tmp, info_tmp):
    ...
    bestEst = 100
    bestMove = None
    for x in figureMoves:
        tmp = setANewState(table_tmp, info_tmp)
        t = estimate(turn, tmp[0], tmp[1], 1)
        if t < bestEst:
            bestEst = t
            bestMove = x
    bestEst = 0
